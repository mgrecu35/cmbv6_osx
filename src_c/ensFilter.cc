...
//#define ran_mod_mp_normal2_() ran_mod_MOD_normal2()
...
extern "C" float ran_mod_MOD_normal2(float *nm, float *nstd); 
...
//  SFM  begin  07/01/2014; for M.Grecu, random sequences
	...
	{
	  float nm=0, nstd=1.;
	  xs[i][j]=__ran_mod_MOD_normal2(&nm,&nstd);
	}
//  SFM  end    07/01/2014
    }
  ...
  for(i=0;i<radarRet->nMemb;i++)
	...
	{
	  float nm=0, nstd=1.;
	  logdNw[i][j]+=0.25*__ran_mod_MOD_normal2(&nm,&nstd);
  ...
  if(stormStruct->rainType==2 and radarData->hfreez>=-2)
	...
	logdNw[i][j]=0.85*logdNw[i][j]+0.15*
	  (0.25*__ran_mod_MOD_normal2(&nm,&nstd));
...
extern "C" void ensradJulia( radarDataType   *radarData, 
                             stormStructType *stormStruct,
                             retParamType    *retParam,  
                             int *nmu, radarRetType    *radarRet, 
                             int *iflag, float *rms1,
...
//  SFM  end    07/01/2014
  ...
  // printf("%i %i %i\n",radarRet->nMemb,nNodes,stormStruct->rainType);
  ...
  for(i=0;i<-9;i++)
    printf("%i ",nodeP[i]);
  //printf("\n");
  //  printf("%i %i %i %g %i %g\n",imuv[1],*ichunk,radarRet->imu[imemb],*localZAngle,*nmu, \
         radarData->dr);
  runEns(radarData, stormStruct,retParam, nmu,radarRet, 
         xscalev, randemiss, localZAngle, wfractPix, ichunk, 
         xs, logdNw,kext,asym,salb,rsurf,dz, imuv, nodeP, 
  ...
  dm/=radarRet->nMemb;
  rsfcMean/=radarRet->nMemb;
  // printf("dm=%g %g %i %g\n",dm,rsfcMean,stormStruct->rainType,*wfractPix);
  //  exit(0);
  if(stormStruct->rainType==2 && dm>0.5 && *wfractPix<10)
    {
      float cdN=0.56826200674023877-0.6907993*dm;
