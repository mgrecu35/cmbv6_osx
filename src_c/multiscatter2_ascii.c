...
int multiscatterdr_(int *nrange, float *extFort, 
                    float *ext2bscatt, float *salbFort, float *gFort,
                    float *bscatFort, int noMS, float dr, float wl)
{
  /* VARIABLE DECLARATIONS */
  ...
  for (i = 0; i < n; i++) {
    /* float *ext2bscatt, float *salbFort, float *gFort,
       float *bscatFort*/
    radius[i]=0.001;
    range[i]=(n-1-i)*dr*1000+dr*1000/2;
    ext[i]=extFort[i];
    ext_bscat_ratio[i]=ext2bscatt[i];
    ext_air[i]=0;
    ssa[i]=salbFort[i];
